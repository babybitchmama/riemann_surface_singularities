...
The \emph{standard metric} $g_0$ can be expressed as $g_0=(\de x)^2+(\de y)^2$ or $g_0=(\de r)^2+r^2(\de\theta)^2$ or $g_0=|\de z|^2$.  This is technically called a $(0,2)$-tensor, but as far as we are concerned it simply represents the method being used to measure the (squared) length of vectors.

Associated to $g_0$ is the \emph{standard symplectic form} $\omega_0=\de x\wedge\de y$ or $\omega_r=\frac1{2i}\de\bar z\wedge\de z$ (we won't need the polar version), which is also a $(0,2)$-tensor which is used to measure the area of parallelograms.  These are mostly used for integration.

As in the stereographic projection example we discussed, sometimes we want to use $\R^2$ to represent points on a surface which may not actually be flat.  This makes the notion of distances between points vary depending on the location in $\R^2$.  Thus, we can used a positive function $u$ on $\R^2$ to create a weighted metric $g=ug_0$.  In the various coordinate systems this looks like\footnote{It is commonplace to use $\de x^2$ to denote $(\de x)^2$; this should not be confused with $\de(x^2)$.}
\begin{align}
    g
        &=u(x,y)\,(\de x^2+\de y^2)\\
...
\begin{figure}\label{fig: Stereographic Projection}
    \centering
    \includegraphics[width=.4\textwidth]{stereographic_projection.png}\qquad
    \includegraphics[width=.3\textwidth]{fubini_study_metric.png}
...
Let $g_t=u_tg_0$ be a family of metrics (the $t$ subscript does not mean derivative---we'll stick with $\partial$ notation for derivatives), and let $K_t=K_{g_t}$.  Let $U\subseteq\R^2$ be a region.  The volume (area) of $U$ is $\mathrm{Vol}_t(U)=\int_U u_t$, and the average curvature on $U$ at time $t$ is $\rho_t=\int_U K_tu_t$.  Notice both values depend on the metric, and therefore depend on time $t$.
