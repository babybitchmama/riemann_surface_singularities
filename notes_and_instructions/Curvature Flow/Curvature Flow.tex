...
Let $g_t=u_tg_0$ be a family of metrics\footnote{The $t$ subscript does not mean derivative.  We'll stick with $\partial$ notation for derivatives.}, and let $K_t=K_{g_t}$.  Let $U\subseteq\R^2$ be a region.  With respect to the metric $g_t$, the volume (area) of $U$ is $\mathrm{Vol}_t(U)=\int_U u_t$ and the average curvature on $U$ is
    $$\rho_t=\frac1{\mathrm{Vol}_t(U)}\int_U K_tu_t.$$
